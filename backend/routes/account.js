...
router.post("/transfer", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    // }
    // const payeeAccount = await Account.findOne({ userId: req.body.to });
    // if (!payeeAccount)
    //   return res.status(400).json({ message: "Invalid account" });
    // payerAccount.balance -= req.body.amount;
    // payeeAccount.balance += req.body.amount;
    // payerAccount.save();
    // payeeAccount.save();

    //with transactions which should be a little more complex and something new,but the ideal method to use
